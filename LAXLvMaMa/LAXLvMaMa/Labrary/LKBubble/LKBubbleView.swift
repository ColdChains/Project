...
class LKBubbleView: UIView {
    ...
    static func laxShowRightWithTitle(title: String, autoCloseTime: CGFloat) -> LKBubbleView  {
        let info: LKBubbleInfo = self.getDefaultRightBubbleInfo()
        info.title = title
        let bubbleView = LKBubbleView()
        bubbleView.showWithInfo(info: info)
        self.perform(#selector(hide), with: self, afterDelay: TimeInterval(autoCloseTime + 0.2))
        return bubbleView
    }
    
    static func laxShowErrorWithTitle(title: String, autoCloseTime: CGFloat) -> LKBubbleView  {
    ...
    static func laxShowRoundProgress() -> LKBubbleView {
    ...
    static func laxShowRightWithTitle(superView: UIView, title: String, autoCloseTime: CGFloat) -> LKBubbleView  {
        let info: LKBubbleInfo = self.getDefaultRightBubbleInfo()
        info.title = title
        let bubbleView = LKBubbleView()
        bubbleView.showWithInfo(info: info, superView: superView)
        self.perform(#selector(hide), with: self, afterDelay: TimeInterval(autoCloseTime + 0.2))
        return bubbleView
    }
    
    static func laxShowErrorWithTitle(superView: UIView, title: String, autoCloseTime: CGFloat) -> LKBubbleView  {
        let info: LKBubbleInfo = self.getDefaultErrorBubbleInfo()
        info.title = title
        let bubbleView = LKBubbleView()
        bubbleView.showWithInfo(info: info, superView: superView)
    ...
    static func laxShowRoundProgress(superView: UIView) -> LKBubbleView {
        let info: LKBubbleInfo = self.getDefaultRoundProgressBubbleInfo()
        info.title = ""
        let bubbleView = LKBubbleView()
        bubbleView.showWithInfo(info: info, superView: superView)
...
//            }
    ...
    func showWithInfo(info: LKBubbleInfo, superView: UIView) -> Void {
        self._currentInfo = info
        self._closeKey = self._currentInfo?.key// 保存当前要关闭的key，防止关闭不需要关闭的bubble
        if info.isShowMaskView {
            //UIApplication.shared.keyWindow?.addSubview(self._maskView)
